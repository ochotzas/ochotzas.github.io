...
export const startHost = (root: HTMLElement) => {
  ...
  QRCode.toDataURL(padUrl, {
    ...
    .then((url) => root.querySelectorAll<HTMLImageElement>("[data-qr]").forEach((el) => (el.src = url)))
    .catch(() => root.querySelectorAll<HTMLImageElement>("[data-qr]").forEach((el) => (el.hidden = true)));
